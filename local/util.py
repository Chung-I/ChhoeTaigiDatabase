...
from local.symbols import 臺灣閩南語羅馬字拼音聲母表
from local.symbols import 臺灣閩南語羅馬字拼音韻母表
from local.symbols import iNULL, TONES, is_phn, all_syls
...
def dict_seg(sent, wordDict):
    min_cuts = [0] + [float('inf') for _ in sent]
    backtrack = [[] for _ in range(len(sent)+1)]
    for i in range(1,len(sent)+1):
        for j in range(0, i):
            if min_cuts[j] < float('inf') and sent[j:i] in wordDict:
                if min_cuts[j] + 1 == min_cuts[i]:
                    backtrack[i].append(j)
                elif min_cuts[j] + 1 < min_cuts[i]:
                    min_cuts[i] = min_cuts[j] + 1
                    backtrack[i] = [j]
    if backtrack[len(sent)] is None:
        #logging.warning(f"{sent} has no proper segmentation")
        return []
    else:
...
def run_g2p():
    import argparse
    from local.dictionary import Dictionary
    parser = argparse.ArgumentParser()
